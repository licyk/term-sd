...
term_sd_setting()
{
    ...
    term_sd_setting_dialog=$(
        dialog --erase-on-exit --notags --title "Term-SD" --backtitle "Term-SD设置选项" --ok-label "确认" --cancel-label "取消" --menu "请选择Term-SD设置" $term_sd_dialog_width $term_sd_dialog_height $term_sd_dialog_menu_height \
        "0" "> 返回" \
...
python_venv_setting()
{
    ...
    case $python_venv_setting_dialog in
        1)
            ...
            dialog --erase-on-exit --title "Term-SD" --backtitle "虚拟环境设置界面" --ok-label "确认" --msgbox "启用成功" $term_sd_dialog_width $term_sd_dialog_height
            python_venv_setting
            ;;
        2)
            venv_setup_status=1
            touch ./term-sd/term-sd-venv-disable.lock
            dialog --erase-on-exit --title "Term-SD" --backtitle "虚拟环境设置界面" --ok-label "确认" --msgbox "禁用成功" $term_sd_dialog_width $term_sd_dialog_height
            python_venv_setting
            ;;
    esac
}
...
pip_mirrors_setting()
{
    ...
    case $pip_mirrors_setting_dialog in
        1)
            ...
            dialog --erase-on-exit --title "Term-SD" --backtitle "pip镜像源(配置文件)选项" --ok-label "确认" --msgbox "设置pip镜像源为官方源成功" $term_sd_dialog_width $term_sd_dialog_height
            pip_mirrors_setting
            ;;
        2)
            ...
            dialog --erase-on-exit --title "Term-SD" --backtitle "pip镜像源(配置文件)选项" --ok-label "确认" --msgbox "设置pip镜像源为国内镜像源成功" $term_sd_dialog_width $term_sd_dialog_height
            pip_mirrors_setting
            ;;
        3)
            ...
            dialog --erase-on-exit --title "Term-SD" --backtitle "pip镜像源(配置文件)选项" --ok-label "确认" --msgbox "删除镜像源配置成功" $term_sd_dialog_width $term_sd_dialog_height
            pip_mirrors_setting
            ;;
    esac
}
...
pip_mirrors_env_setting()
{
    ...
    case $pip_mirrors_env_setting_dialog in
        1)
            ...
            dialog --erase-on-exit --title "Term-SD" --backtitle "pip镜像源(环境变量)选项" --ok-label "确认" --msgbox "设置pip镜像源为官方源成功" $term_sd_dialog_width $term_sd_dialog_height
            pip_mirrors_env_setting
            ;;
        2)
            ...
            dialog --erase-on-exit --title "Term-SD" --backtitle "pip镜像源(环境变量)选项" --ok-label "确认" --msgbox "设置pip镜像源为国内镜像源成功" $term_sd_dialog_width $term_sd_dialog_height
            pip_mirrors_env_setting
            ;;
    esac
}
...
term_sd_proxy_setting()
{
    ...
    case $term_sd_proxy_setting_dialog in
        ...
        3)
            ...
            ;;
        4)
            if (dialog --erase-on-exit --title "Term-SD" --backtitle "代理参数删除界面" --yes-label "是" --no-label "否" --yesno "是否删除代理配置?" $term_sd_dialog_width $term_sd_dialog_height) then
                http_proxy=
                https_proxy=
                rm -f ./term-sd/proxy.conf
                dialog --erase-on-exit --title "Term-SD" --backtitle "代理参数设置界面" --ok-label "确认" --msgbox "清除设置代理完成" $term_sd_dialog_width $term_sd_dialog_height
            fi
            term_sd_proxy_setting
            ;;
    esac
...
term_sd_watch_setting()
{
    ...
    case $term_sd_watch_setting_dialog in
        1)
            term_sd_watch_value=$(dialog --erase-on-exit --title "Term-SD" --backtitle "命令执行监测设置界面" --ok-label "确认" --cancel-label "取消" --inputbox "请输入重试次数(仅输入数字,不允许输入负数和其他非数字的字符)" $term_sd_dialog_width $term_sd_dialog_height "$(cat ./term-sd/term-sd-watch-retry.conf)" 3>&1 1>&2 2>&3)
            if [ ! -z "$(echo $term_sd_watch_value | awk '{gsub(/[0-9]/, "")}1')" ];then
                dialog --erase-on-exit --title "Term-SD" --backtitle "命令执行监测设置界面" --ok-label "确认" --msgbox "输入格式错误,重试次数只能为数字且不能为负数" $term_sd_dialog_width $term_sd_dialog_height
            else
                echo "$term_sd_watch_value" > ./term-sd/term-sd-watch-retry.conf
                term_sd_cmd_retry=$term_sd_watch_value
                dialog --erase-on-exit --title "Term-SD" --backtitle "命令执行监测设置界面" --ok-label "确认" --msgbox "启用成功" $term_sd_dialog_width $term_sd_dialog_height
            fi
            term_sd_watch_setting
            ;;
        2)
            ...
            term_sd_cmd_retry=0
            dialog --erase-on-exit --title "Term-SD" --backtitle "命令执行监测设置界面" --ok-label "确认" --msgbox "禁用成功" $term_sd_dialog_width $term_sd_dialog_height
            term_sd_watch_setting
            ;;
    esac
}
...
term_sd_install_mode_setting()
{
    ...
    case $term_sd_install_mode_setting_dialog in
        1)
            ...
            dialog --erase-on-exit --title "Term-SD" --backtitle "安装模式设置界面" --ok-label "确认" --msgbox "启用严格模式成功" $term_sd_dialog_width $term_sd_dialog_height
            term_sd_install_mode_setting
            ;;
        2)
            term_sd_install_mode=1
            touch ./term-sd/term-sd-disable-strict-install-mode.lock
            dialog --erase-on-exit --title "Term-SD" --backtitle "安装模式设置界面" --ok-label "确认" --msgbox "启用宽容模式成功" $term_sd_dialog_width $term_sd_dialog_height
            term_sd_install_mode_setting
            ;;
    esac
}
...
aria2_multi_threaded_setting()
{
    ...
    case $aria2_multi_threaded_setting_dialog in
        1)
            aria2_multi_threaded_value=$(dialog --erase-on-exit --title "Term-SD" --backtitle "aria2线程设置界面" --ok-label "确认" --cancel-label "取消" --inputbox "请输入线程数(仅输入数字(),不允许输入负数和其他非数字的字符)" $term_sd_dialog_width $term_sd_dialog_height "$(cat ./term-sd/aria2-thread.conf | awk '{sub("-x ","")}1')" 3>&1 1>&2 2>&3)
            if [ ! -z "$(echo $aria2_multi_threaded_value | awk '{gsub(/[0-9]/, "")}1')" ] || [ $aria2_multi_threaded_value = 0 ] ;then
                dialog --erase-on-exit --title "Term-SD" --backtitle "aria2线程设置界面" --ok-label "确认" --msgbox "输入格式错误,线程数只能为数字且不能为负数" $term_sd_dialog_width $term_sd_dialog_height
            else
                if [ $aria2_multi_threaded_value -le 16 ];then
                    ...
                    dialog --erase-on-exit --title "Term-SD" --backtitle "aria2线程设置界面" --ok-label "确认" --msgbox "启用成功" $term_sd_dialog_width $term_sd_dialog_height
                else
        ...
        2)
            ...
            dialog --erase-on-exit --title "Term-SD" --backtitle "aria2线程设置界面" --ok-label "确认" --msgbox "禁用成功" $term_sd_dialog_width $term_sd_dialog_height
            aria2_multi_threaded_setting
            ;;
    esac
}
...
cuda_memory_alloc_setting()
{
    ...
    case $cuda_memory_alloc_setting_dialog in
        1)
            ...
            dialog --erase-on-exit --title "Term-SD" --backtitle "CUDA内存分配设置界面" --ok-label "确认" --msgbox "设置CUDA内存分配器为PyTorch原生分配器成功" $term_sd_dialog_width $term_sd_dialog_height
            cuda_memory_alloc_setting
            ;;
        2)
            PYTORCH_CUDA_ALLOC_CONF=backend:cudaMallocAsync
            echo "backend:cudaMallocAsync" > ./term-sd/cuda-memory-alloc.conf
            dialog --erase-on-exit --title "Term-SD" --backtitle "CUDA内存分配设置界面" --ok-label "确认" --msgbox "设置CUDA内存分配器为CUDA内置异步分配器成功" $term_sd_dialog_width $term_sd_dialog_height
            cuda_memory_alloc_setting
            ;;
        3)
            PYTORCH_CUDA_ALLOC_CONF=
            rm -f ./term-sd/cuda-memory-alloc.conf
            dialog --erase-on-exit --title "Term-SD" --backtitle "CUDA内存分配设置界面" --ok-label "确认" --msgbox "清除CUDA内存分配器设置成功" $term_sd_dialog_width $term_sd_dialog_height
            cuda_memory_alloc_setting
            ;;
    esac
}
...
" $term_sd_dialog_width $term_sd_dialog_height
}
