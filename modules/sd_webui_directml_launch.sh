...
sd_webui_directml_launch_args_setting()
{
    ...
    sd_webui_directml_launch_args_setting_dialog=$(
        dialog --erase-on-exit --notags --title "Stable-Diffusion-WebUI管理" --backtitle "Stable-Diffusion-WebUI-DirectML启动参数选项" --ok-label "确认" --cancel-label "取消" --checklist "请选择Stable-Diffusion-WebUI-DirectML启动参数,确认之后将覆盖原有启动参数配置" $term_sd_dialog_width $term_sd_dialog_height $term_sd_dialog_menu_height \
...
sd_webui_directml_launch_args_revise()
{
    ...
    sd_webui_directml_launch_args=$(dialog --erase-on-exit --title "Stable-Diffusion-WebUI管理" --backtitle "Stable-Diffusion-WebUI-DirectML自定义启动参数选项" --ok-label "确认" --cancel-label "取消" --inputbox "请输入Stable-Diffusion-WebUI-DirectML启动参数" $term_sd_dialog_width $term_sd_dialog_height "$(cat ./term-sd-launch.conf | awk '{sub("launch.py ","")}1')" 3>&1 1>&2 2>&3)
