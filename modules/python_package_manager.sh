...
python_package_manager()
{
    ...
    python_package_name=$(dialog --erase-on-exit --title "Term-SD" --backtitle ""$term_sd_manager_info" pip软件包安装/重装/卸载选项" --ok-label "确认" --cancel-label "取消" --inputbox "请输入需要安装/重装/卸载的pip软件包名" 25 80 3>&1 1>&2 2>&3)
    if [ $? = 0 ] && [ ! -z "$(echo $python_package_name | awk '{gsub(/[=+<>]/, "")}1')" ];then
        ...
        case $pip_manage_package_methon in # 选择pip包管理器管理方法
            1) # 常规安装
                term_sd_watch term_sd_pip install $python_package_name $pip_index_mirror $pip_extra_index_mirror $pip_find_mirror $pip_break_system_package $pip_install_mode --default-timeout=100 --retries 5
                ;;
            2) # 仅安装
                term_sd_watch term_sd_pip install --no-deps $python_package_name $pip_index_mirror $pip_extra_index_mirror $pip_find_mirror $pip_break_system_package $pip_install_mode --default-timeout=100 --retries 5
                ;;
            3) # 强制重装
                term_sd_watch term_sd_pip install --force-reinstall $python_package_name $pip_index_mirror $pip_extra_index_mirror $pip_find_mirror $pip_break_system_package $pip_install_mode --default-timeout=100 --retries 5
                ;;
            4) # 仅强制重装
                term_sd_watch term_sd_pip install --force-reinstall --no-deps $python_package_name $pip_index_mirror $pip_extra_index_mirror $pip_find_mirror $pip_break_system_package $pip_install_mode --default-timeout=100 --retries 5
                ;;
            5) # 卸载
...
python_package_update()
{
    ...
    # 更新python软件包
    term_sd_watch term_sd_pip install -r ./tmp-python-package-update-list.txt --prefer-binary --upgrade $pip_index_mirror $pip_extra_index_mirror $pip_find_mirror $pip_break_system_package $pip_break_system_package --default-timeout=100 --retries 5
