    ...
    do
        ...
        case $comfyui_extension_interface_dialog in
            1)
                ...
                case $? in
                    0)
                        dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI插件更新结果" --ok-label "确认" --msgbox "${comfyui_extension_name}插件更新成功" $term_sd_dialog_height $term_sd_dialog_width
                        ;;
                    10)
                        dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI插件更新结果" --ok-label "确认" --msgbox "${comfyui_extension_name}插件非git安装,无法更新" $term_sd_dialog_height $term_sd_dialog_width
                        ;;
                    *)
                        dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI插件更新结果" --ok-label "确认" --msgbox "${comfyui_extension_name}插件更新失败" $term_sd_dialog_height $term_sd_dialog_width
                        ;;
                esac
                ;;
            2)
                if (dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI插件修复更新" --yes-label "是" --no-label "否" --yesno "是否修复${comfyui_extension_name}自定义节点更新?" $term_sd_dialog_height $term_sd_dialog_width) then
                    git_fix_pointer_offset
                fi
                [ $? = 10 ] && dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI插件修复更新" --ok-label "确认" --msgbox "${comfyui_extension_name}插件非git安装,无法修复更新" $term_sd_dialog_height $term_sd_dialog_width
                ;;
            ...
            4)
                git_ver_switch
                [ $? = 10 ] && dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI插件版本切换" --ok-label "确认" --msgbox "${comfyui_extension_name}插件非git安装,无法进行版本切换" $term_sd_dialog_height $term_sd_dialog_width
                ;;
            5)
                git_remote_url_select_single
                [ $? = 10 ] && dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI插件更新源切换" --ok-label "确认" --msgbox "${comfyui_extension_name}插件非git安装,无法进行更新源切换" $term_sd_dialog_height $term_sd_dialog_width
                ;;
            6)
                if (dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI插件删除选项" --yes-label "是" --no-label "否" --yesno "是否删除${comfyui_extension_name}插件?" $term_sd_dialog_height $term_sd_dialog_width) then
                    ...
                    case $(term_sd_read) in
                        yes|y|YES|Y)
                            ...
                            dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI插件删除选项" --ok-label "确认" --msgbox "删除$(basename "$comfyui_custom_node_name")插件完成" $term_sd_dialog_height $term_sd_dialog_width
                            break
                            ;;
                        *)
                            term_sd_echo "取消删除操作"
