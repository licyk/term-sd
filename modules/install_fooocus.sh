    ...
    cat<<EOF
{
    ...
    "Softness of ControlNet": "ControlNet控制权重",
    ...
    "Same as the denoising strength in A1111 inpaint. Only used in inpaint, not used in outpaint. (Outpaint always use 1.0)": "该选项和A1111 SD WebUI中重绘功能的重绘幅度相同。该选项仅应用于图生图重绘功能中，在文生图中该设置无效（在文生图中该值为1.0）",
    "Inpaint Respective Field": "重绘蒙版区域范围",
    "The area to inpaint. Value 0 is same as \"Only Masked\" in A1111. Value 1 is same as \"Whole Image\" in A1111. Only used in inpaint, not used in outpaint. (Outpaint always use 1.0)": "调整重绘区域的范围。该值为0时和A1111 SD WebUI中”重绘区域“选项的”仅蒙版区域“的效果相同，为1时和”整张图片“效果相同。该选项仅应用于图生图重绘功能中，在文生图中该设置无效（在文生图中该值为1.0）",
    ...
    "* Powered by Fooocus Inpaint Engine": "* 由Fooocus重绘引擎驱动",
    "Describe": "图像提示词反推",
    ...
    "Metadata": "图片信息查看",
    "Drag any image generated by Fooocus here": "导入由Fooocus生成的图片",
    ...
    "Disable automatic seed increment when image number is > 1.": "当图片生成批次大于1时禁用种子增量",
    ...
    "Image Prompt parameters are not included. Use png and a1111 for compatibility with Civitai.": "使用默认设置时图片提示词参数不包括在内。使用png图片保存格式和A1111 SD WebUI的图片信息保存风格的图片更适合在Civitai进行分享。",
    "fooocus (json)": "fooocus风格（json）",
    "a1111 (plain text)": "A1111 SD WebUI风格（纯文本）",
    "Extreme Speed": "极速",
    "Lightning": "闪速",
    "Refiner Switch At": "Refind切换时机",
    "Use 0.4 for SD1.5 realistic models; or 0.667 for SD1.5 anime models; or 0.8 for XL-refiners; or any value for switching two SDXL models.": "SD1.5真实模型使用0.4，SD1.5动漫模型为0.667，XLRefind机为 0.8，或用于切换两个SDXL模型的任何值。",
