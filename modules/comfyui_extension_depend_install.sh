...
function comfyui_extension_depend_install_process()
{
    ...
    for i in ./*;do
        ...
        if [ -f "./install.py" ];then #找到install.py文件
            cmd_daemon python_cmd install.py
            if [ $? = 0 ];then #记录退出状态
        ...
        if [ -f "./requirements.txt" ];then #找到requirement.txt文件
            cmd_daemon pip_cmd install -r requirements.txt
            if [ $? = 0 ];then #记录退出状态
    ...
    tmp_enable_proxy
    print_line_to_shell
    dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI"$comfyui_extension_info_display"依赖安装结果" --ok-label "确认" --msgbox "当前依赖的安装情况列表\n------------------------------------------------------------------\n$dep_info------------------------------------------------------------------" 25 80
}
...
function comfyui_extension_depend_install_single()
{
    ...
    exit_venv
    dialog --erase-on-exit --title "ComfyUI选项" --backtitle "ComfyUI"$comfyui_extension_info_display"依赖安装结果" --ok-label "确认" --msgbox "当前依赖的安装情况列表\n------------------------------------------------------------------\n$dep_info------------------------------------------------------------------" 25 80
}