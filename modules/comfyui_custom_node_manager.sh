...
comfyui_custom_node_manager()
{
    ...
    comfyui_custom_node_manager_dialog=$(
        dialog --erase-on-exit --notags --title "ComfyUI管理" --backtitle "ComfyUI自定义节点管理选项" --ok-label "确认" --cancel-label "取消" --menu "请选择ComfyUI自定义节点管理选项的功能" $term_sd_dialog_width $term_sd_dialog_height $term_sd_dialog_menu_height \
        "0" "> 返回" \
        "1" "> 安装" \
        "2" "> 管理" \
        "3" "> 更新全部自定义节点" \
...
comfyui_custom_node_install()
{
    ...
    comfyui_custom_node_url=$(dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI自定义节点安装选项" --ok-label "确认" --cancel-label "取消" --inputbox "输入自定义节点的github地址或其他下载地址" $term_sd_dialog_width $term_sd_dialog_height 3>&1 1>&2 2>&3)
    ...
    if [ ! -z $comfyui_custom_node_url ]; then
        ...
        if [ $git_req = 0 ];then
            dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI自定义节点安装结果" --ok-label "确认" --msgbox "$(echo $comfyui_custom_node_url | awk -F'/' '{print $NF}')自定义节点安装成功\n$comfyui_custom_node_dep_notice" $term_sd_dialog_width $term_sd_dialog_height
        else
            dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI自定义节点安装结果" --ok-label "确认" --msgbox "$(echo $comfyui_custom_node_url | awk -F'/' '{print $NF}')自定义节点安装失败" $term_sd_dialog_width $term_sd_dialog_height
        fi
    fi
}
...
comfyui_custom_node_list()
{
    ...
    comfyui_custom_node_name=$(
        dialog --erase-on-exit --yes-label "确认" --no-label "取消" --title "ComfyUI管理" --backtitle "ComfyUI自定义节点列表" --menu "使用上下键选择要操作的自定义节点并回车确认" $term_sd_dialog_width $term_sd_dialog_height $term_sd_dialog_menu_height \
        "-->返回<--" "<---------" \
...
comfyui_custom_node_interface() 
{
    ...
    case $comfyui_custom_node_interface_dialog in
        1)
            ...
            case $? in
                0)
                    dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI自定义节点更新结果" --ok-label "确认" --msgbox "${comfyui_custom_node_name}自定义节点更新成功" $term_sd_dialog_width $term_sd_dialog_height
                    ;;
                10)
                    dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI自定义节点更新结果" --ok-label "确认" --msgbox "${comfyui_custom_node_name}自定义节点非git安装,无法更新" $term_sd_dialog_width $term_sd_dialog_height
                    ;;
                *)
                    dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI自定义节点更新结果" --ok-label "确认" --msgbox "${comfyui_custom_node_name}自定义节点更新失败" $term_sd_dialog_width $term_sd_dialog_height
                    ;;
            esac
            comfyui_custom_node_interface
            ;;
        2)
            ...
            [ $? = 10 ] && dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI自定义节点修复更新" --ok-label "确认" --msgbox "${comfyui_custom_node_name}自定义节点非git安装,无法修复更新" $term_sd_dialog_width $term_sd_dialog_height
            comfyui_custom_node_interface
            ;;
        ...
        4)
            git_ver_switch
            [ $? = 10 ] && dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI自定义节点版本切换" --ok-label "确认" --msgbox "${comfyui_custom_node_name}自定义节点非git安装,无法进行版本切换" $term_sd_dialog_width $term_sd_dialog_height
            comfyui_custom_node_interface
            ;;
        5)
            git_remote_url_select_single
            [ $? = 10 ] && dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI自定义节点更新源切换" --ok-label "确认" --msgbox "${comfyui_custom_node_name}自定义节点非git安装,无法进行更新源切换" $term_sd_dialog_width $term_sd_dialog_height
            comfyui_custom_node_interface
            ;;
        6)
            if (dialog --erase-on-exit --title "ComfyUI选项" --backtitle "ComfyUI自定义节点删除选项" --yes-label "是" --no-label "否" --yesno "是否删除${comfyui_custom_node_name}自定义节点?" $term_sd_dialog_width $term_sd_dialog_height) then
