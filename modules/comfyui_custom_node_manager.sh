...
comfyui_custom_node_install()
{
    ...
    comfyui_custom_node_url=$(dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI自定义节点安装选项" --ok-label "确认" --cancel-label "取消" --inputbox "输入自定义节点的github地址或其他下载地址" 25 80 3>&1 1>&2 2>&3)
    ...
    if [ ! -z $comfyui_custom_node_url ]; then
        ...
        if [ $git_req = 0 ];then
            ...
            dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI自定义节点安装结果" --ok-label "确认" --msgbox "$(echo $comfyui_custom_node_url | awk -F'/' '{print $NF}')自定义节点安装失败" 25 80
        fi
    fi
}
...
comfyui_custom_node_list()
{
    ...
    comfyui_custom_node_name=$(
        dialog --erase-on-exit --yes-label "确认" --no-label "取消" --title "ComfyUI管理" --backtitle "ComfyUI自定义节点列表" --menu "使用上下键选择要操作的自定义节点并回车确认" 25 80 10 \
        "-->返回<--" "<---------" \
...
comfyui_custom_node_interface() 
{
    ...
    case $comfyui_custom_node_interface_dialog in
        1)
            ...
            case $? in
                0)
                    dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI自定义节点更新结果" --ok-label "确认" --msgbox "${comfyui_custom_node_name}自定义节点更新成功" 25 80
                    ;;
                10)
                    dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI自定义节点更新结果" --ok-label "确认" --msgbox "${comfyui_custom_node_name}自定义节点非git安装,无法更新" 25 80
                    ;;
                *)
                    dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI自定义节点更新结果" --ok-label "确认" --msgbox "${comfyui_custom_node_name}自定义节点更新失败" 25 80
                    ;;
            esac
            comfyui_custom_node_interface
            ;;
        2)
            ...
            [ $? = 10 ] && dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI自定义节点修复更新" --ok-label "确认" --msgbox "${comfyui_custom_node_name}自定义节点非git安装,无法修复更新" 25 80
            comfyui_custom_node_interface
            ;;
        ...
        4)
            git_ver_switch
            [ $? = 10 ] && dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI自定义节点版本切换" --ok-label "确认" --msgbox "${comfyui_custom_node_name}自定义节点非git安装,无法进行版本切换" 25 80
            comfyui_custom_node_interface
            ;;
        5)
            git_remote_url_select_single
            [ $? = 10 ] && dialog --erase-on-exit --title "ComfyUI管理" --backtitle "ComfyUI自定义节点更新源切换" --ok-label "确认" --msgbox "${comfyui_custom_node_name}自定义节点非git安装,无法进行更新源切换" 25 80
            comfyui_custom_node_interface
            ;;
        6)
            if (dialog --erase-on-exit --title "ComfyUI选项" --backtitle "ComfyUI自定义节点删除选项" --yes-label "是" --no-label "否" --yesno "是否删除${comfyui_custom_node_name}自定义节点?" 25 80) then
