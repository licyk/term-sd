...
comfyui_custom_node_manager()
{
    ...
    do
        ...
        comfyui_custom_node_manager_dialog=$(dialog --erase-on-exit --notags \
            --title "ComfyUI 管理" \
...
comfyui_custom_node_install()
{
    ...
    comfyui_custom_node_url=$(dialog --erase-on-exit \
        --title "ComfyUI 管理" \
    ...
    if [ ! -z "$comfyui_custom_node_url" ]; then
        ...
        --ok-label "确认" \
        --msgbox "$comfyui_custom_node_dep_notice" $term_sd_dialog_height $term_sd_dialog_width

        comfyui_custom_node_dep_notice=
    else
        term_sd_echo "输入的 ComfyUI 自定义节点安装地址为空, 不执行操作"
    fi
}
...
comfyui_custom_node_list()
{
    ...
    do
        ...
        comfyui_custom_node_name=$(dialog --erase-on-exit \
...
comfyui_custom_node_interface() 
{
    ...
    while true
    do
        comfyui_custom_node_interface_dialog=$(dialog --erase-on-exit --notags \
            --title "ComfyUI 选项" \
            --backtitle "ComfyUI 自定义节点管理选项" \
            --ok-label "确认" --cancel-label "取消" \
            --menu "请选择对 ${comfyui_custom_node_name} 自定义节点的管理功能\n当前更新源: $(git_remote_display)\n当前分支: $(git_branch_display)" \
            $term_sd_dialog_height $term_sd_dialog_width $term_sd_dialog_menu_height \
            "0" "> 返回" \
            "1" "> 更新" \
            "2" "> 修复更新" \
        ...
        case $comfyui_custom_node_interface_dialog in
            ...
            6)
                if (dialog --erase-on-exit \
                    ...
                    --yes-label "是" --no-label "否" \
                    --yesno "是否删除 ${comfyui_custom_node_name} 自定义节点?" \
                    $term_sd_dialog_height $term_sd_dialog_width) then
                    ...
                    case $(term_sd_read) in
                        yes|y|YES|Y)
                            ...
                            --backtitle "ComfyUI 自定义节点删除选项" \
                            --ok-label "确认" \
                            --msgbox "删除 $(basename "$comfyui_custom_node_name") 自定义节点完成" $term_sd_dialog_height $term_sd_dialog_width
