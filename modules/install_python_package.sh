...
function reinstall_python_packages()
{
    ...
    if [ ! -z "$(echo $reinstall_python_packages_names | awk '{gsub(/[=+<>]/, "")}1')" ];then
        ...
        if [ $? = 0 ];then
            print_line_to_shell
            dialog --clear --title "Term-SD" --backtitle "pip软件包安装结果" --ok-label "确认" --msgbox ""$term_sd_manager_info"安装成功" 25 70
        else
            print_line_to_shell
            dialog --clear --title "Term-SD" --backtitle "pip软件包安装结果" --ok-label "确认" --msgbox ""$term_sd_manager_info"安装失败" 25 70
        fi
    fi
}