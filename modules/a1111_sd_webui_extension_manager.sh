...
function a1111_sd_webui_extension_methon()
{
    ...
    a1111_sd_webui_extension_methon_dialog=$(
        dialog --clear --title "A1111-SD-Webui管理" --backtitle "A1111-SD-Webui插件管理选项" --ok-label "确认" --cancel-label "取消" --menu "请选择A1111-SD-Webui插件管理选项的功能" 25 80 10 \
        "1" "安装" \
        "2" "管理" \
        "3" "更新全部插件" \
        "4" "返回" \
...
function a1111_sd_webui_extension_manager()
{
    ...
    extension_selection=$(
        dialog --clear --ok-label "确认" --cancel-label "取消" --title "A1111-SD-Webui管理" --backtitle "A1111-SD-Webui插件列表" --menu "请选择A1111-SD-Webui插件" 25 80 10 \
        "-->返回<--" "<---------" \
        $dir_list \
        3>&1 1>&2 2>&3)
...
{
    extension_address=$(dialog --clear --title "A1111-SD-Webui管理" --backtitle "A1111-SD-Webui插件安装选项" --ok-label "确认" --cancel-label "取消" --inputbox "请输入插件的github地址或其他下载地址" 25 80 3>&1 1>&2 2>&3)
    ...
    if [ ! -z $extension_address ]; then
        ...
        if [ $? = 0 ];then
            dialog --clear --title "A1111-SD-Webui管理" --backtitle "A1111-SD-Webui插件安装结果" --ok-label "确认" --msgbox "$(echo $extension_address | awk -F'/' '{print $NF}')插件安装成功" 25 80
        else
            dialog --clear --title "A1111-SD-Webui管理" --backtitle "A1111-SD-Webui插件安装结果" --ok-label "确认" --msgbox "$(echo $extension_address | awk -F'/' '{print $NF}')插件安装失败" 25 80
        fi
    fi
}
    ...
    if [ $? = 0 ];then
        case $a1111_sd_webui_operate_extension_dialog in
            1)
                ...
                if [ $? = 0 ];then
                    dialog --clear --title "A1111-SD-Webui管理" --backtitle "A1111-SD-Webui插件更新结果" --ok-label "确认" --msgbox ""$extension_selection"插件更新成功" 25 80
                else
                    dialog --clear --title "A1111-SD-Webui管理" --backtitle "A1111-SD-Webui插件更新结果" --ok-label "确认" --msgbox ""$extension_selection"插件更新失败" 25 80
                fi
                a1111_sd_webui_operate_extension
                ;;
            2)
                if (dialog --clear --title "A1111-SD-Webui管理" --backtitle "A1111-SD-Webui插件删除选项" --yes-label "是" --no-label "否" --yesno "是否删除"$extension_selection"插件?" 25 80) then
