...
function requirements_python_package_update()
{
    ...
    for ignore_update_python_package_list_ in $ignore_update_python_package_list; do
        sed -i '/'$ignore_update_python_package_list_'/d' ./tmp-python-package-update-list.txt 2> /dev/null  #将忽略的软件包从名单删除
    done
    #更新依赖
