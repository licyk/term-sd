...
\n
### 3、设置安装重试功能（可选）\n
3、因为安装重试功能是在命令行中操作，所以先退出Term-SD，然后在启动Term-SD的命令后面加上“--set-cmd-daemon-retry”，再回车启动Term-SD，这时就设置好安装重试功能了，在安装ai软件时如果遇到网络不稳定导致命令执行的中断时，将会重新执行中断的命令（默认重试3次，可参照说明中的[“启动参数”](#启动参数)页面来修改），保证安装的顺利进行\n
\n
