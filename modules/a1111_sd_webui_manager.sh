...
function a1111_sd_webui_option()
{
    ...
    if [ -d "stable-diffusion-webui" ];then #找到stable-diffusion-webui目录
        ...
        a1111_sd_webui_option_dialog=$(
            dialog --erase-on-exit --title "A1111-SD-Webui管理" --backtitle "A1111-SD-Webui管理选项" --ok-label "确认" --cancel-label "取消" --menu "请选择A1111-SD-Webui管理选项的功能\n当前更新源:$(git remote -v | awk 'NR==1 {print $2}')\n当前分支:$sd_webui_branch_info" 25 80 10 \
            "1" "更新" \
            "2" "卸载" \
            "3" "修复更新" \
            "4" "管理插件" \
            ...
            case $a1111_sd_webui_option_dialog in
                1)
                    ...
                    if [ $? = 0 ];then
                        ...
                        dialog --erase-on-exit --title "A1111-SD-Webui管理" --backtitle "A1111-SD-Webui更新结果" --ok-label "确认" --msgbox "A1111-SD-Webui更新失败" 25 80
                    fi
                    a1111_sd_webui_option
                    ;;
                2)
                    if (dialog --erase-on-exit --title "A1111-SD-Webui管理" --backtitle "A1111-SD-Webui删除选项" --yes-label "是" --no-label "否" --yesno "是否删除A1111-Stable-Diffusion-Webui?" 25 80) then
                ...
                6)
                    ...
                    ;;
                7)
                    if (dialog --erase-on-exit --title "A1111-SD-Webui管理" --backtitle "A1111-SD-Webui分支切换选项" --yes-label "是" --no-label "否" --yesno "是否切换Stable-Diffusion-Webui分支?" 25 80) then
                ...
                9)
                    ...
                    ;;
                10)
                    if (dialog --erase-on-exit --title "A1111-SD-Webui管理" --backtitle "A1111-SD-Webui重新安装选项" --yes-label "是" --no-label "否" --yesno "是否重新安装A1111-Stable-Diffusion-Webui?" 25 80) then
                        cd "$start_path"
                        exit_venv
                ...
                13)
                    ...
                    ;;
                18)
                    if (dialog --erase-on-exit --title "A1111-SD-Webui管理" --backtitle "A1111-SD-Webui虚拟环境修复选项" --yes-label "是" --no-label "否" --yesno "是否修复A1111-SD-Webui的虚拟环境" 25 80);then
                        create_venv --fix
                    fi
                    a1111_sd_webui_option
                    ;;
                19)
                    if (dialog --erase-on-exit --title "A1111-SD-Webui管理" --backtitle "A1111-SD-Webui虚拟环境重建选项" --yes-label "是" --no-label "否" --yesno "是否重建A1111-SD-Webui的虚拟环境" 25 80);then
        ...
        if (dialog --erase-on-exit --title "A1111-SD-Webui管理" --backtitle "A1111-SD-Webui安装选项" --yes-label "是" --no-label "否" --yesno "检测到当前未安装A1111-Stable-Diffusion-Webui,是否进行安装?" 25 80) then
            process_install_a1111_sd_webui
        fi
    fi
}
...
{
    if (dialog --erase-on-exit --title "A1111-SD-Webui管理" --backtitle "A1111-SD-Webui依赖更新选项" --yes-label "是" --no-label "否" --yesno "是否更新A1111-Stable-Diffusion-Webui的依赖?" 25 80);then
        #更新前的准备
        proxy_option #代理选择
