...
function python_package_ver_backup_or_restore()
{
    ...
    enter_venv
    python_package_ver_backup_or_restore_dialog=$(dialog --erase-on-exit --title "Term-SD" --backtitle "依赖库版本管理选项" --ok-label "确认" --cancel-label "取消" --menu "请选择Term-SD的依赖库版本管理功能\n当前"$term_sd_manager_info"依赖库版本备份情况:$( [ ! -z "$(ls ./term-sd-python-pkg-backup)" ] && echo \\n$( ls -lrh ./term-sd-python-pkg-backup --time-style=+"%Y-%m-%d" | awk 'NR==2 {print $7}' ) || echo "无" )" 25 80 10 \
        "1" "备份python依赖库版本" \
        "2" "python依赖库版本管理" \
        "3" "返回" \
        3>&1 1>&2 2>&3)
...
{
    process_python_package_ver_backup_file_dialog=$(dialog --erase-on-exit --title "Term-SD" --backtitle "依赖库版本记录管理选项" --ok-label "确认" --cancel-label "取消" --menu "请选择Term-SD的依赖库版本记录管理功能\n当前版本记录:\n$(echo $python_package_ver_backup_list_dialog | awk '{sub(".txt","")}1')" 25 80 10 \
        "1" "恢复该版本记录" \
        "2" "删除该版本记录" \
        "3" "返回" \
        3>&1 1>&2 2>&3)
